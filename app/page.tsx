  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      {showSplash && (
        ...
        >
          <div className="text-center max-w-md w-full px-4 sm:px-6">
            <h1
              className="text-4xl sm:text-6xl md:text-8xl font-bold text-white mb-4 tracking-wider"
              style={{
            ...
            <div
              className="w-12 sm:w-16 h-1 bg-white mx-auto rounded-full mb-6 sm:mb-8"
              style={{
                animation: 'expandWidth 1.8s ease-out 0.8s both'
              }}
            ></div>
            <div
              className="mb-6 sm:mb-8 text-white/80 text-base sm:text-lg font-light"
              style={{
            ...
            >
              <div>
                <input
                  ...
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 rounded-lg bg-white/10 backdrop-blur-sm text-white placeholder:text-white/60 border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/30 focus:border-white/40 transition-all duration-200 text-sm sm:text-base"
                  disabled={isValidating}
                />
                {emailError && (
                  <p className="text-red-300 text-xs sm:text-sm mt-2 text-left">
                    {emailError}
                  </p>
                )}
              </div>
              ...
              <button
                type="submit"
                disabled={isValidating}
                className="w-full px-4 sm:px-6 py-2 sm:py-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white font-medium rounded-lg border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/30 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
              >
      ...
      <div id="app" className={`${showSplash ? 'opacity-0 scale-105' : 'opacity-100 scale-100'} transition-all duration-1500 ease-in-out`}>
        {/* Navigation */}
        <header
          className={`absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-4 sm:px-8 py-4 sm:py-6 opacity-0 ${isLoaded ? "animate-fade-in" : ""}`}
          style={{ animationDelay: "0.2s" }}
        >
          <div className="flex items-center gap-2 sm:gap-4">
            <Menu className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
            <span className="text-lg sm:text-2xl font-semibold text-white drop-shadow-lg">Agend-IA</span>
            {userName && (
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold shadow-md text-sm sm:text-base">
              U
            </div>
          </div>
        </header>
        ...
        {/* Main Content */}
        <main className="relative h-screen w-full pt-20 flex flex-col lg:flex-row">
          {/* Sidebar */}
          <div
            className={`w-full lg:w-64 h-auto lg:h-full bg-white/10 backdrop-blur-lg p-4 lg:p-4 shadow-xl border-b lg:border-r border-white/20 rounded-t-3xl lg:rounded-tr-3xl opacity-0 ${isLoaded ? "animate-fade-in" : ""} flex flex-col justify-between`}
            style={{ animationDelay: "0.4s" }}
          >
            <div>
              <button
                onClick={openEventModal}
                className="mb-4 sm:mb-6 flex items-center justify-center gap-2 rounded-full bg-blue-500 px-3 sm:px-4 py-2 sm:py-3 text-white w-full hover:bg-blue-600 transition-colors text-sm sm:text-base"
              >
              ...
              <div className="mb-4 sm:mb-6">
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <h3 className="text-white font-medium text-sm sm:text-base">{currentMonthFormatted}</h3>
                ...
                <div className="grid grid-cols-7 gap-1 text-center">
                  ...
                  {miniCalendarDays.map((day, i) => (
                    <div
                      key={i}
                      className={`text-xs rounded-full w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center cursor-pointer transition-colors ${day === Number(format(currentDate, "d")) ? "bg-blue-500 text-white" : "text-white hover:bg-white/20"
                        } ${!day ? "invisible" : ""}`}
                      onClick={() => {
                        if (day) {
                  ...
                  {selectedDayTasks.length === 0 ? (
                    ...
                    selectedDayTasks.map((task: { id: string, text: string, completed: boolean }) => (
                      <div key={task.id} className="flex items-center gap-2 sm:gap-3">
                        <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-sm ${task.completed ? 'bg-green-500' : 'bg-blue-500'}`}></div>
                        <span className={`text-white text-xs sm:text-sm ${task.completed ? 'line-through opacity-50' : ''}`}>
                          {task.text}
                        </span>
                      </div>
                    ))
                ...
                <div className="space-y-2">
                  {(() => {
                    ...
                    return dayEvents.map((event) => (
                      <div key={event.id} className="flex items-center gap-2 sm:gap-3 p-2 rounded-lg bg-white/10">
                        <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-sm ${event.color}`}></div>
                        <div className="flex-1">
                          <div className="text-white text-xs sm:text-sm font-medium">{event.title}</div>
          ...
          >
            {/* Calendar Controls */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border-b border-white/20 gap-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 w-full sm:w-auto">
                <button
                  className="px-3 sm:px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600 transition-colors text-sm sm:text-base"
                  onClick={goToToday}
                >
                  Today
                </button>
                ...
                <h2 className="text-lg sm:text-xl font-semibold text-white">{currentDateFormatted}</h2>
              </div>

              <div className="flex items-center gap-1 sm:gap-2 rounded-md p-1">
                <button
                  onClick={goToDayView}
                  className={`px-2 sm:px-3 py-1 rounded text-xs sm:text-sm ${currentView === "day" ? "bg-white/20" : ""} text-white`}
                >
                  Day
                </button>
                <button
                  onClick={goToWeekView}
                  className={`px-2 sm:px-3 py-1 rounded text-xs sm:text-sm ${currentView === "week" ? "bg-white/20" : ""} text-white`}
                >
                  Week
                </button>
                <button
                  onClick={goToMonthView}
                  className={`px-2 sm:px-3 py-1 rounded text-xs sm:text-sm ${currentView === "month" ? "bg-white/20" : ""} text-white`}
                >
                  Month
                </button>
              </div>
            ...
            {showPlanning ? (
              <div className="flex-1 overflow-auto p-2 sm:p-4">
                <div className="bg-white/20 backdrop-blur-lg rounded-xl border border-white/20 shadow-xl h-full p-4 sm:p-6">
                  {/* Header del Planning */}
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-4">
                    ...
                    <button
                      ...
                      className="px-3 sm:px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors flex items-center gap-2 text-sm sm:text-base"
                    >
                  ...
                  <form onSubmit={addTask} className="mb-4 sm:mb-6">
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                      <input
                        ...
                        className="flex-1 px-3 sm:px-4 py-2 sm:py-3 rounded-lg bg-white/10 backdrop-blur-sm text-white placeholder:text-white/60 border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/30 focus:border-white/40 transition-all duration-200 text-sm sm:text-base"
                      />
                      <button
                        type="submit"
                        className="px-4 sm:px-6 py-2 sm:py-3 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-lg transition-colors text-sm sm:text-base"
                      >
                        Añadir
                      </button>
                    </div>
              ...
              <div className="flex-1 overflow-auto p-2 sm:p-4">
                <div className="bg-white/20 backdrop-blur-lg rounded-xl border border-white/20 shadow-xl min-h-screen">
                  ...
                  <div className="grid grid-cols-8 border-b border-white/20">
                    ...
                    {weekDays.map((day, i) => (
                      <div key={i} className="p-1 sm:p-2 text-center border-l border-white/20">
                        <div className="text-xs text-white/70 font-medium">{weekDayNames[i]}</div>
                        <div
                          className={`text-sm sm:text-lg font-medium mt-1 text-white ${format(day, "d") === format(currentDate, "d") &&
                  ...
                  <div className="grid grid-cols-8">
                    ...
                    <div className="text-white/70">
                      {timeSlots.map((time, i) => (
                        <div key={i} className="h-16 sm:h-20 border-b border-white/10 pr-1 sm:pr-2 text-right text-xs">
                    ...
                    {Array.from({ length: 7 }).map((_, dayIndex) => (
                      <div key={dayIndex} className="border-l border-white/20 relative">
                        {timeSlots.map((_, timeIndex) => (
                          <div key={timeIndex} className="h-16 sm:h-20 border-b border-white/10"></div>
                        ))}
          ...
          {showAIPopup && (
            <div className="fixed bottom-4 sm:bottom-8 right-4 sm:right-8 z-20">
              <div className="w-[300px] sm:w-[450px] relative bg-gradient-to-br from-blue-400/30 via-blue-500/30 to-blue-600/30 backdrop-blur-lg p-4 sm:p-6 rounded-2xl shadow-xl border border-blue-300/30 text-white">
                <button
                ...
                <div className="mt-4 sm:mt-6 flex gap-2 sm:gap-3">
                  <button
                    onClick={togglePlay}
                    className="flex-1 py-2 sm:py-2.5 bg-white/10 hover:bg-white/20 rounded-xl text-xs sm:text-sm transition-colors font-medium"
                  >
                    Yes
                  </button>
                  <button
                ...
                {isPlaying && (
                  <div className="mt-4 flex items-center justify-between">
                    <button
                      className="flex items-center justify-center gap-2 rounded-xl bg-white/10 px-3 sm:px-4 py-2 sm:py-2.5 text-white text-xs sm:text-sm hover:bg-white/20 transition-colors"
                      onClick={togglePlay}
                    >
                      <Pause className="h-3 w-3 sm:h-4 sm:w-4" />
                      <span className="hidden sm:inline">Pause Hans Zimmer</span>
      ...
      {showEventModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white/10 backdrop-blur-lg p-4 sm:p-6 rounded-xl shadow-xl max-w-md w-full mx-2 sm:mx-4 border border-white/20 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEventSubmit} className="space-y-3 sm:space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-white/30 text-sm sm:text-base"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-white/30 text-sm sm:text-base"
                  />
                </div>
                <div>
                  <label className="block text-white text-sm font-medium mb-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4">
                <button
                  type="button"
                  onClick={closeEventModal}
                  className="flex-1 px-3 sm:px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors text-sm sm:text-base"
                >
                  Cancelar
                </button>
                <button
